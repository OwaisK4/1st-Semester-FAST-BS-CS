...
int main()
{
	...
	FILE *fa;
	system("color 70");
		fa=fopen("E:\\FAST NU assignments\\PF Project\\FINAL PROJECT COMPLETE\\data.txt","r");
		...
		for(i=1;i<2;i++){
				...
				printf("%d%s%s%s%s%d%d%d%d",d[i].id,d[i].name,d[i].dob,d[i].phoneno,d[i].nationality,d[i].pincode,d[i].bankbalance,d[i].deposit,d[i].withdraw);
		if(i==ide)
			{
				if(pc==d[i].pincode)
				{
